...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if login_attempt:
        if login_attempt.lock_until and login_attempt.lock_until > datetime.now(UTC):
            raise HTTPException(
            ...
            login_attempt.lock_until = datetime.now(UTC) + timedelta(minutes=30)
            db.commit()
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    if not verify_password(password, user.password):
        ...
        login_attempt.last_failed_attempt = datetime.now(UTC)
        db.commit()
