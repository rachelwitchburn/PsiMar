...
async def confirm_payment(payment_id: int, db: Session = Depends(get_db)):
    payment = payment_service.update_payment_status(db, payment_id, status="paid")
    if not payment:
    ...
    content = f"Olá {payment.patient.first_name}, seu pagamento de R$ {payment.amount:.2f} foi confirmado com sucesso!"
