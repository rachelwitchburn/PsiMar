...
async def get_appointments(db: Session = Depends(get_db), is_professional: bool = True, current_user: User = Depends(get_current_user)):
    """
       Retorna a lista de agendamentos associados a um usuário.

    """
    user_id = current_user.id
    appointments = crud.appointment_service.appointment_list(db=db, user_id=user_id, is_professional=is_professional)
    if not appointments:
