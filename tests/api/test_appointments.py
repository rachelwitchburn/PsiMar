...
def test_appointment_conflict(client, create_test_user, login_test_user, unique_email, unique_access_code):
    ...
    future_datetime = (datetime.now(timezone.utc) + timedelta(days=1)).replace(microsecond=0).isoformat()
...
def test_patient_requests_and_professional_confirms_appointment(client, create_test_user, login_test_user, unique_email, unique_access_code):
    # Cria paciente e faz login como paciente
    patient_id = create_test_user("Paciente", "User", unique_email, "123456", user_type="patient")
    patient_token = login_test_user(unique_email, "123456")
    ...
    professional_token = login_test_user(professional_email, "123456")
    professional_headers = {"Authorization": f"Bearer {professional_token}"}

    # Profissional confirma o agendamento
    confirm_response = client.post(
        f"/appointment/confirm-professional/{appointment_id}",
        headers=professional_headers
    )

    print(confirm_response.json())

    assert confirm_response.status_code == 200, f"Esperado status 200 na confirmação, recebido {confirm_response.status_code}: {confirm_response.text}"
    confirm_data = confirm_response.json()
    assert confirm_data["status"] == "confirmed", f"Status esperado 'confirmed', recebido: {confirm_data['status']}"
    assert confirm_data[
               "id"] == appointment_id, "O ID do agendamento confirmado deve ser o mesmo do agendamento solicitado."