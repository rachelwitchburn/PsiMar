...
from app.database_app import SessionLocal  # Importando a função SessionLocal para obter uma sessão de banco de dados
from app.models import Usuario  # Importando o modelo Usuario para interagir com a tabela 'users' no banco de dados
from app.security import create_access_token, verify_password, get_current_usuario  # Importando funções de segurança
...
def login(email: str, senha: str, db: Session = Depends(get_db)):
    ...
    access_token = create_access_token(data={"sub": usuario.email, "is_admin": usuario.is_admin}, expires_delta=access_token_expires)
...
def reset_password(nova_senha: str, current_usuario: Usuario = Depends(get_current_usuario), db: Session = Depends(get_db)):
    """
