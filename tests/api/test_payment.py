...
def test_duplicate_payment_not_allowed(client, create_test_user, login_test_user, create_payment_payload, unique_email,
                                       unique_access_code, insert_access_code):
    # Cria paciente e faz login
    patient_id = create_test_user("Login", "User", unique_email, "123456", user_type="patient")
    token = login_test_user(unique_email, "123456")
    headers = {"Authorization": f"Bearer {token}"}
    ...
    # Tentar realizar segundo pagamento para o mesmo agendamento
    response = client.post("/payments/", json=payload, headers=headers)

    # Espera-se que a API rejeite a duplicação de pagamento
    assert response.status_code == 400 or response.status_code == 409, f"Pagamento duplicado aceito indevidamente: {response.status_code}, {response.text}"