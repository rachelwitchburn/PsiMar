...
def confirm_appointment(page: ft.Page):
    ...
    if response.status_code == 200:
        ...
        if not agendamentos_pendentes:
            ...
            for ag in agendamentos_pendentes:
                ...
                def confirmar_click(e, agendamento_id=agendamento_id):
                    resp = api.confirm_appointment_by_professional(agendamento_id)

                    if resp.status_code == 200:
                        print(f" Agendamento {agendamento_id} confirmado.")
                        page.go("/professional_confirm_appointment")  # Atualiza a tela
                    else:
    ...
    return ft.View(
        route="/professional_confirm_appointment",
        controls=[
            ft.Row(
                controls=[
                    ft.Container(
