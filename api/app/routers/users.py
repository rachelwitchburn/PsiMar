...
def login(email: str, senha: str, db: Session = Depends(get_db)):
    ...
    if login_attempt:
        ...
        if login_attempt.lock_until and login_attempt.lock_until > datetime.datetime.utcnow():
            ...
            login_attempt.lock_until = datetime.datetime.utcnow() + datetime.timedelta(minutes=30)
            db.commit()
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    if not verify_password(senha, usuario.senha):
        ...
        login_attempt.last_failed_attempt = datetime.datetime.utcnow()
        db.commit()
